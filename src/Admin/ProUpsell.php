...
final class ProUpsell
{
    ...
    public function enabled(): bool
    {
        ...
        if (defined('Tipping\\Pro\\VERSION')) {
            return false;
        }
        /**
    ...
    private function priceLabel(): string
    {
        ...
        if (! empty($d['price_from'])) {
            ...
            return sprintf(__('from %1$s%2$d/yr', 'dankon'), $cur, (int) $d['price_from']);
        }
        return '';
    }
    ...
    public function handleDismiss(): void
    {
        if (! current_user_can('manage_woocommerce')) {
            wp_die(esc_html__('Permission denied.', 'dankon'));
        }
    ...
    public function banner(): void
    {
        ...
        $name     = (string) ($this->data()['name'] ?? 'Dankon Pro');
        ...
        <div class="tipping-pro-banner" role="note">
            ...
            <p class="tipping-pro-banner__text">
                ...
                printf(esc_html__('Do more with %s', 'dankon'), esc_html($name)); ?></strong>
            ...
            <a class="tipping-pro-banner__dismiss" href="<?php echo esc_url($this->dismissUrl()); ?>" aria-label="<?php esc_attr_e('Dismiss this notice', 'dankon'); ?>">&times;</a>
        </div>
        <?php
    }
    ...
    public function aside(): void
    {
        ...
        <aside class="tipping-pro-aside" aria-labelledby="tipping-pro-aside-h">
            ...
            <h2 id="tipping-pro-aside-h" class="tipping-pro-aside__heading"><?php esc_html_e('Unlock every PRO feature', 'dankon'); ?></h2>
            ...
            <?php if ($price !== '') : ?>
                <p class="tipping-pro-aside__price"><?php echo esc_html($price); ?><?php if ($this->sellable()) : ?> · <?php esc_html_e('one licence, every PRO feature', 'dankon'); ?><?php endif; ?></p>
            <?php endif; ?>
        </aside>
        <?php
    }
    ...
    public function cards(): void
    {
        ...
        <section class="tipping-pro-cards" aria-labelledby="tipping-pro-cards-h">
            <h2 id="tipping-pro-cards-h" class="tipping-pro-cards__title">
                <?php
                /* translators: %s: PRO edition name */
                printf(esc_html__('What %s adds', 'dankon'), esc_html($name)); ?>
            </h2>
